...
int GameScene::Object_num()
{
    return this->gameObjects.size();
}
bool GameScene::Scene_empty()
{
    return this->gameObjects.empty();
}
void GameScene::Clear_it()//GameObject *except_for
{
    for (auto gameObject : gameObjects)
    {
        //if(gameObject != except_for)
        this->detachGameObject(gameObject);
    }
}
void GameScene::Stop_all()
{
    Q_ASSERT(this->updateTimer->isActive());
    this->updateTimer->stop();
}
void GameScene::Reck_all()
{
    Q_ASSERT(!this->updateTimer->isActive());
